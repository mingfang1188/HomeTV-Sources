...
频道表只在用户明确决定维护时，由家中 Mac 手动运行 `scripts/update_manual.command` 检查并提交，不设置任何定时任务。这样测速路径与电视所在地区一致，也不会让海外云主机误删只在中国可用的频道。检测包含播放清单、最高画质变体、真实视频分片、解码信息和下载速度余量。只发布无需账号、Cookie 或私有凭证的公开地址。
